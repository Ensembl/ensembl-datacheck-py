...
def test_get_ftp_paths_returns_paths_from_metadata_fixture():
    ...
    if "homologies" in path_by_dataset_type:
        assert re.match(common_prefix + r"/homology$", path_by_dataset_type["homologies"])
    ...
    # prefix of the others, it's an equal peer.
    ...
    if "homologies" in path_by_dataset_type:
        common_path_candidates.append(path_by_dataset_type["homologies"].removesuffix("/homology"))
    if "regulation" in path_by_dataset_type:
