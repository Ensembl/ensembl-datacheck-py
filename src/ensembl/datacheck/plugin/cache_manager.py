...
class CacheManager:
    ...
    def get_cache_dir(self):
        ...
        if self.file_path:
            ...
            file_hash = xxhash.xxh64_hexdigest(
                pathlib.Path(self.file_path).read_bytes()
            )
            cache_dir = pathlib.Path(
                f"/hps/nobackup/flicek/ensembl/production/datachecks/python_dc/{file_hash}"
            )
            ...
            cache_dir = pathlib.Path(
                f"/hps/nobackup/flicek/ensembl/production/datachecks/python_dc/{server}/{db_name}/{last_update_str}"
            )
        else:
    ...
    def load_test_results(self):
        ...
        if results_file.exists():
            ...
            raise FileNotFoundError(
                f"No previous test results found for {self.file_path or self.database_url}"
            )
    ...
    def handle_cache_post_run(self, terminalreporter):
        ...
        This includes writing the summary to the results file, saving the cache, and cleaning up if
        all tests passed.
    ...
    def write_summary_to_file(self, results_file):
        ...
        """
        custom_summary_plugin = self.config.pluginmanager.getplugin(
            "custom_summary_plugin"
        )
