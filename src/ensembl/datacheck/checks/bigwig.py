...
This module performs basic BigWig checks.

Checks performed:
1. check_exist: Asserts that the target BigWig path exists.
...
def check_validity(target_file):
    """
    Check that the target file is recognised as BigWig.

    Args:
        target_file (pathlib.Path or None): Path to the target file.

    Raises:
        AssertionError: If the file is missing, unreadable, or not BigWig.
    """
    assert file_exists(target_file), "The target file does not exist."
    reader = None
    try:
    ...
    try:
        ...
        for chrom in chroms:
            ...
            for s_i in range(0, end, window):
                ...
                values = np.array(reader.values(chrom, s_i, e_i))
